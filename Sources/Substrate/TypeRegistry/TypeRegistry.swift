...
extension TypeRegistry: TypeRegistryProtocol {
    ...
    public func decode(keyFrom decoder: ScaleDecoder) throws -> AnyStorageKey {
        ...
        let _info = try info(forKey: prefix)
        if let kt = _keys["\(_info.module).\(_info.field)"] {
            return try _decodingError {
                try kt.init(parsingPathFrom: decoder, registry: self)
            }
        } else {
            return try _decodingError  {
                try DStorageKey(module: _info.module, field: _info.field, decoder: decoder, registry: self)
            }
        }
    }
    ...
    public func decode(eventFrom decoder: ScaleDecoder) throws -> AnyEvent {
        ...
        let _info = try info(forEvent: header)
        if let event = _events["\(_info.module).\(_info.event)"] {
            return try _eventDecodingError(module: _info.module, event: _info.event) {
                try event.init(decodingDataFrom: decoder, registry: self)
            }
        } else {
            return try _eventDecodingError(module: _info.module, event: _info.event) {
                try DEvent(module: _info.module, event: _info.event, decoder: decoder, registry: self)
            }
        }
    }
    ...
    public func decode(callFrom decoder: ScaleDecoder) throws -> AnyCall {
        ...
        let _info = try info(forCall: header)
        if let call = _calls["\(_info.module).\(_info.call)"] {
            return try _callDecodingError(module: _info.module, funct: _info.call) {
                try call.init(decodingParamsFrom: decoder, registry: self)
            }
        } else {
            return try _callDecodingError(module: _info.module, funct: _info.call) {
                try DCall(module: _info.module, function: _info.call, decoder: decoder, registry: self)
            }
        }
    }
}
