...
extension DoubleMapStorageKey {
    ...
    public func encode(in encoder: ScaleEncoder, registry: TypeRegistryProtocol) throws {
        ...
        try encoder.encode(Self._prefix(), .fixed(Self.prefixSize))
        let _hash1 = try Self._hash(hasher: hashers[0], hash: hash1, key: key1, registry: registry)
        let _hash2 = try Self._hash(hasher: hashers[1], hash: hash2, key: key2, registry: registry)
        try encoder.encode(_hash1, .fixed(UInt(_hash1.count)))
        try encoder.encode(_hash2, .fixed(UInt(_hash2.count)))
    }
